    ...
    public class ReportServiceBD : IReportService
    {
        ...
        public void SaveDvigateliPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var Dvigatelis = context.Dvigatelis.ToList();
                //создаем таблицу
                var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, Dvigatelis.Count, 2, ref missing, ref missing);
                ...
                for (int i = 0; i < Dvigatelis.Count; ++i)
                {
                    table.Cell(i + 1, 1).Range.Text = Dvigatelis[i].DvigateliName;
                    table.Cell(i + 1, 2).Range.Text = Dvigatelis[i].Price.ToString();
                }
        ...
        public List<GarazhsLoadViewModel> GetGarazhsLoad()
        {
            return context.Garazhs
                            .ToList()
                            .GroupJoin(
                                    context.GarazhDetalis
                                                .Include(r => r.Detali)
                                                .ToList(),
                                    Garazh => Garazh,
                                    GarazhDetali => GarazhDetali.Garazh,
                                    (Garazh, GarazhCompList) =>
            new GarazhsLoadViewModel
            {
            ...
            try
            {
                ...
                if (dict != null)
                {
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        excelcells.Value2 = elem.GarazhName;
        ...
        public void SaveZakazchikZakazs(ReportBindingModel model)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].DvigateliName, fontForCells));
                table.AddCell(cell);
